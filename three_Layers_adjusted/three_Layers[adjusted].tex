...
The standard way of studying \eqref{intro_slow} is using methods from geometric singular perturbation theory. An breif overview of this theory invloves treating system \eqref{intro_slow}, which is refered to as the slow-system, together with its counterpart, the fast-system:
...
where if $\tau$ denotes the (slow) time varibale in system \eqref{intro_slow}, then $t = \tau/\eps$ is the (fast) time varible used in system \eqref{intro_fast}.
...
\]
Therefore, we will use \eqref{euler_ori_eqn} for analysis in the rest of the paper, also we drop the tilde to use $f(u,\mu,\eps)$ to denote the new rescaled nonlinearity.
...
The solution to \eqref{ori_eqn} will be based on the solution of the Riccati equation \eqref{ric}. We need to rescale time $t$ to obtain appropriate equations as $\eps \to 0$.
...
\begin{proof}
From the asymptotics \eqref{ricasy}, we have that
\[
...
Recall the critical manifold for system \eqref{crit_mfld} is the set of points $(u,\mu) \in \mathbb{R}^2$ which satisfies
\begin{equation} \label{crit_mfld}
\mu + u^2 + f(u,\mu; 0) =  0.
\end{equation}

By the implicit function theorem, whenever $\partial_u( u^2+f(u,\mu;0) ) \neq 0$, we can write $u=u(\mu),$ for $\mu$ small, let $u_s(t):= u(\mu(t))$ so that
...
It can be calculated that $u_s(t)$ has the following asymptotics
...
\pagebreak
\section*{Appendix}
In this Appendix we show the main perturbation lemma used to prove the invertibility of the linearized operators at the ansatzs.
