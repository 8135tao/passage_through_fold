...
In conclusion, the boundary value problem we wish to solve is
...
\]
which is defined on a dense subset $\mathcal{D}$ of $\mathcal{C}_r$.

\begin{proposition}\label{inv_A_r}
The linear operator $A_r(u_0,\eps) : \mathcal{D}\subset \mathcal{C}_r \to \mathcal{C}_r\times \mathbb{R}$ and is invertible, with its inverse 
\[
...
Then, from equation \eqref{asy:a} we see that the first component of $\tilde{A}_r$ converges uniformly in $u_0$ and $\eps$ to the asymptotic operator 
\[
