...
\begin{figure}[ht]
 ...
 \label{fig:passage} % labeling to refer it inside the text
...
\begin{figure}[ht]
 ...
 \label{fig:time_scale} % labeling to refer it inside the text
...
For each $\delta>0$ and $\eta>0$ small, there exist $\eps_0$ and $u_0$, and parameter $u_i= u_i(\eps,\delta)$, $T=T(\eps,u_0)$ such that for all $0<\eps<\eps_0$ and $|u_0 - \bar{u}_0|<\eta$, a solution of equation \ref{euler_ori_eqn} exists with $\mu(t) = \eps t - \delta$ and 
\begin{align*}
u(t; u_0)  =\begin{cases}
...
\end{cases}
\end{align*}
where $u_A(t; u_0)$, $u_B(t)$ and $u_C(t)$ are the ansatz mentioned earlier. Moreover, we have
\[
T(\eps,u_0) = \eps^{-1}\delta + \eps^{-1/3}\Omega_\infty(u_0) -\delta^{-1} + \rmO(\eps^{2/3}), \hspace{0.2in} u(T;u_0) = \delta,
\]
and
\[
u_i = -\sqrt{\delta} +\delta + \rmO( \eps^{1-\alpha/3}\delta^{-1}), \hspace{0.2in} u(0;u_0)  =u_i.
\]
\end{theorem}
