...
% we have that the nonhomogeneous term is of $\rmO_\eps(1)$ in the $w_-$ equation, consistent with the Region $1$ norm.


%Then we get the $w_-$ equation in the $\sigma$ variable.
%\begin{align*}
%\frac{d}{d\sigma} w_- &= 2\eps^{-1/3}\varphi[\chi_+u_++\chi_-u_-]w_- +\eps^{-2/3}\varphi[(\chi_-\chi_+)u_-(u_--u_+)+\frac{1}{2}\chi_-'(u_+-u_-)]\\
%& + \eps^{2/3}\varphi w_-^3+\eps^{-2/3}\varphi\chi_-^3u_-^3+3\varphi(\chi_-u_-)w_-^2+3\eps^{-1/3}\varphi(\chi_-u_-)^2w_-\\
%&+3\eps^{-2/3}\varphi (\chi_-u_-+W_-)^2\chi_+u_+ + 3\eps^{-1/3}\varphi [(\chi_+u_++W_+)^2-W_+^2]w_-\\
%&+\eps^{-1/3}\varphi[2+3(W_-+W_+) ]\chi_+W_+w_-
%\end{align*}
...
%Change to $\xi$ time
%\begin{align*}
%\frac{d}{d\xi} W_+ &= e^{-\xi}\chi_+ \mu + 2e^{-\xi}[\chi_+u_++\chi_- u_-]W_+ + e^{-\xi}[ \chi_-\chi_+ u_+(u_--u_+)+\frac{1}{2}\chi_+'(u_+-u_-)] \\
%& + e^{-\xi}(\chi_+^3u_+^3+W_+^3)+3e^{-\xi}[\chi_+u_+W_+^2+(\chi_+u_+)^2W_+ ]\\
%&3e^{-\xi} \{ (\chi_+u_++W_+)^2\chi_-u_- + [(\chi_-u_-+W_-)^2-W_-^2]W_+ \}\\
%&+e^{-\xi}[2 + 3(W_-+W_+)]\chi_-W_-W_+
%\end{align*}
...
%Now we can set up the fixed point equation in appropirate function space, for the $w_-$ equation we work with the function space $C^1(0,\infty)$ in the $\sigma$ variable (weight $\eps^{-1/3}$ with $C^1(-\infty,0)$?). For the $W_+$ equation, we need to use the weight $e^{-(2-\eta)}$ in $C^1$ norm in the $\xi$ variable, this implies that $W_+(t)$ will behave like $u_+^{2-\eta}(t)$ near the gluing point and $W_-$ is bounded with order $\eps^{1/3}$.
...
\item $(u_--u_+)$ terms.
\begin{theorem} \label{DifrEsti}
If $|t-t_*|\le C$ for some positive number $C$, we have 
\[
|u_-(t)-u_+(t)| \le C'\eps^{2/3}
\] 
where $C'$ is another constant depending on $C$.
\end{theorem}
...
\end{align*}
For $|t-t_*|\le C$, we have $(u_-+u_+)(t) \le C\eps^{1/6}$ for some constant $C$.

Hence by Grownwall we have that
\[
v(t) \le C' \left[\frac{1}{2}(t^2-t_*^2)-\delta(t-t_*)\right] \le C'\eps^{2/3}
\]
\end{proof}
...
satisfies
\[
\| \mathcal{R}_{-,d} \|_{\mathcal{C}_{W}}= o_{\eps}(1)
\]
\end{theorem}
\begin{proof}
For $t\in \supp (\chi_-\chi_+) = \supp \chi_-'$, we have $|t-t_*| \le 1$. Note $u_-(t_*) = \rmO(\eps^{1/6})$ and $|u_-(t)-u_-(t_*)| = \rmO(\eps^{1/3})$ from mean value theorem, from previous theorem we conclude that
\[
| (\chi_-\chi_+)u_-(u_--u_+)|_\infty = \rmO(\eps^{\frac{5}{6}}).
\]
 and
 \[
 | \chi_-'(u_+-u_-) |_\infty= \rmO(\eps^{\frac{2}{3}}).
 \]
 
 The third term $2\chi_-(u_- -u_+)W_+$, however, only gets multiplied by $\chi_-$. We cannot just estimate this term in a compact interval around the gluing point $t_*$. Instead we will utilize the following facts:
\begin{itemize}
\item for $0<t<\eps^{-1}\delta$, we have
\[
 |u_-(t)| \le C(\sqrt{\delta-\eps t}+\eps^{1/3})
 \]
 \item for $\eps^{-1}\delta < t <t_*$, we have
 \[
 |u_-(t)|\le C|u_+(t)|
 \]
\end{itemize}
The constant $C$ in the above statements are independent of $\eps, \delta$.

To see how these statements proves the theorem, we estimate, for $\eps^{-1}\delta<t<t_*$ we have $I(t) = \eps^{-\frac{1-\nu}{3}} (T_\infty - t)^{1+\nu}$,
\begin{align*}
|I(t)  \chi_-(u_+ - u_-)W_+|_\infty &\le |I(t) \chi_-u_+ W_+|_\infty\\
&+|I(t) \chi_-u_- W_+|_\infty.
\end{align*}
 
Since $|u_+| \le C(T_\infty -t)^{-1}$, and the estimate $|W_+| \le C(T_\infty-t)^{-\eta}$ holds, we have, for the first of the two parts
\[
|I(t)\chi_-u_+ W_+|_\infty \le |\eps^{-\frac{1-\nu}{3}} (T_\infty-t)^{\nu - \eta} |_\infty \le \eps^{-\frac{1-\nu}{3}} (T_\infty-t_*)^{\nu - \eta} = \delta^{2(\eta-1)}.
\]
...
Similarly, for $0<t<\eps^{-1}\delta$, where the weight $I(t) $ equals $[\eps^{\frac{3}{2}}(\eps^{-1}\delta -t)^{\frac{3}{2}}+\eps^{\frac{2}{3}}]^{-1}$.

We again have to estimate $|I(t)\chi_- u_+ W_+|_\infty$ and $|I(t)\chi_- u_- W_+|_\infty$, first we have
\begin{align*}
|I(t)\chi_- u_+ W_+|_\infty &\le I(t)(T_\infty - t)^{-(1+\eta)} \le I(\eps^{-1}\delta)(T_\infty - \eps^{-1}\delta)^{-(1+\eta)} \\
&\le C\eps^{-\frac{2}{3}} \eps^{\frac{1+\eta}{3}} = \rmO(\eps^{\frac{\eta-1}{3}} )
\end{align*}

For the $u_-$ term, we use the claim to get
\begin{align*}
|I(t)\chi_- u_- W_+|_\infty  \le \frac{\sqrt{\delta-\eps t}+\eps^{1/3}}{ (\delta-\eps t)^{\frac{3}{2}} + \eps^{2/3}} (T_\infty - \eps^{-1}\delta)^{-\eta} \le \eps^{-1/3}\eps^{\frac{\eta}{3}} = \rmO(\eps^{\frac{\eta-1}{3}} )
\end{align*}
...
\begin{proof}
For $t > \eps^{-1}\delta$, near the gluing point, we have
\[ 
\|W_-^2\|_{\mathcal{C}_V} \le \|W_-\|_{C_v} |(T_\infty-t)W_- |_{\infty} = \rmO(\eps^{\frac{1-\nu}{6}})\|W_-\|_{\mathcal{C}_v}
\] 

For $t < \eps^{-1}\delta$, let $\tau = \eps^{-1}\delta-t$, then
\[
\|W_-^2\|_{\mathcal{C}_V}  \le \left| [\eps \tau +\eps^{2/3}]^{-1} [\eps\tau+\eps^{\frac{1}{3}}]^2\right|_{\infty} = \rmO(\delta)
\]
what about norms in the middle? that is near $\eps^{-1}\delta$, seems I can only get $\rmO(1)$ bound here.
...
////////////estimate for $W_+$ equation////////////
\begin{align*}
W_+'  &=  \chi_+\mu+2[u_+ +\chi_-(u_- -u_+)]W_+ + (\chi_-\chi_+) u_+(u_- -u_+)+\frac{1}{2}\chi_+'(u_- -u_+) +2\chi_+W_-W_+\\
&+W_+^2+ (\chi_+u_+ + W_+)^3 \\
&+ 3\chi_+^2\chi_-u_+^2u_- + 6\chi_+\chi_- u_+u_- W_+ +6 \chi_+u_+W_+W_- +3\chi_+(W_-+W_+)W_-W_+\\
&+3(\chi_-u_-)W_+^2 + 3(\chi_-u_-)^2W_+
\end{align*}
...
\begin{proof}
The two term $(\chi_-\chi_+)u_+(u_- - u_+)$ and $\frac{1}{2}\chi_+'(u_- - u_+)$ have compact support inside $(t_*-1, t_* + 1)$. Again, recall $u_+(t_*)  = \rmO(\eps^{1/6}) $ and $|u_- - u_+| = \rmO(\eps^{2/3})$, for all $t$ with $|t-t_*| \le 1$, we have that
\[
|(\chi_-\chi_+)u_+(u_- - u_+) + \frac{1}{2}\chi_+'(u_- - u_+) |_\infty= \rmO(\eps^{5/6}).
\]

For the linear term $\chi_-(u_- - u_+)W_+$, we calculate
\[
\|\chi_-(u_+ - u_-) W_+ \|_{\eta+1} \le |(T_\infty - t)(u_+-u_-) |_\infty \|W_+\|_\eta
\]
again use $u_+ - u_- = o(\frac{1}{T_\infty - t} )$.
\end{proof}
...
First, we have
\[
\|W_+^2\|_{\eta+1} = |(T_\infty -t )^{1+\eta}W_+^2|_\infty \le (T_\infty - t)^{1+\eta}(T_\infty - t)^{-2\eta}\|W_+ \|^2_{\eta} \le (T_\infty - t)^{1-\eta}\|W_+ \|^2_{\eta}
\]
now since $t \in (0,T)$ we have $|T_\infty -t|  = \rmO(\eps^{-1})$, this gives
\[
\|W_+^2\|_{\eta+1} = \rmO(\eps^{\eta-1})
\]
since $\eta >1$, this is desired estimate.

Next we have
\[
\|(\chi_+u_+)^3\|_{\eta+1} = | (T_\infty - t)^{1+\eta} u_+^3 |_\infty \le C(T_\infty -t)^{1+\eta}(T_\infty -t)^{-3} = \rmO(\eps^{2-\eta})
\]
this uses $\eta<2$, again, is desired.

Finally, for the cubic term
\[
\|W_+^3\|_{\eta+1} \le \|W_+\|^3_{\eta} (T_\infty -t)^{1+\eta -3\eta}  = \rmO(\eps^{2\eta-1})
\]
\end{proof}
\item mixed terms
...
\end{align*}
 ...
 \begin{proof}
The first two terms $3(\chi_+^2\chi_-)u_+^2u_-$ and $6(\chi_+\chi_-)u_+u_-W_+$ are localized around $(t_*-1,t_*+1)$, hence we can treat them the same way as in the $\mathcal{R}_{-,m}$ case, using theorem \ref{DifrEsti} 

The middle term $6\chi_+(3^{-1}+u_++2^{-1}(W_-+W_+))W_-W_+$ is estimated as follows:

We only need to consider the term $\chi_+W_-W_+$, and compute
\[
\|\chi_+W_-W_+ \|_{\eta+1} \le |(T_\infty -t)W_-|_\infty \|W_+\|_\eta \le [\eps^{\frac{1}{3}}(T_\infty -t)]^{1-\nu} \|W_+\|_\eta
\]
because of the cut off $\chi_+$, we know that $t \in (t_*-1, T)$, so $|T_\infty - t| = \rmO(\eps^{-\frac{1}{6}})$, this shows
\[
\|W_-W_+\|_{\eta+1} = \rmO(\eps^{\frac{1-\nu}{6}})
\]

Finally, the last two terms $3(\chi_-u_-)W_+^2$ and $3(\chi_-u_-)^2 W_+$ is treated as follows:

For $t$ in the support of $\chi_-$, we have $|u_-| \le C\delta^{\frac{1}{2}}$, so
\[
\|(\chi_-u_-)W_+^2\|_{\eta+1} \le |u_-| \|W_+^2\|_{\eta+1},
\]
where $\|W_+^2\|_{\eta+1}$ is estimated before.

For $3(\chi_-u_-)^2 W_+$, we have
\[
\|3(\chi_-u_-)^2 W_+\|_{\eta+1} = |(T_\infty - t)(\chi_-u_-)^2|_\infty \|W_+\|_\eta
\]
same question, we need to use $u_- - u_+ = o(u_+)$ through out...
 \end{proof}
\end{enumerate}
