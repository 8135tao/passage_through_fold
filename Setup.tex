...
\]
The Lipschitz constant will be of order $e^{-\xi}w \sim e^{(1-\eta)\xi}$, which is small on the relevant interval $\xi_* \le \xi \le 0$. \fi
...
\[
 ...
 \]
\end{enumerate}
In particular, since $|u_+(t) | \lar (T_\infty- t)^{-1}$, we have $|u_-(t)| \lar 1$ for $0<t<t_*$ by picking $\delta$ small and letting $\eps \to 0$.
\end{lemma}
...
\]
 The third term $2\chi_-(u_- -u_+)W_+$, however, only gets multiplied by $\chi_-$. We cannot just estimate this term in a compact interval around the gluing point $t_*$, instead, we need to estimate separately on the interval $(\eps^{-1}\delta,t_*)$ and $(0,\eps^{-1}\delta)$. 

For $\eps^{-1}\delta<t<t_*$ where $I(t) = \delta^{-\frac{1}{4}}\eps^{\frac{\nu-1}{3}} (T_\infty - t)^{1+\nu}$, it follows that
\begin{align*}
...
\[
|I(t)\chi_-u_+ W_+|_\infty \lar |\delta^{-\frac{1}{4}}\eps^{\frac{\nu-1}{3}} (T_\infty-t)^{\nu - \eta} |_\infty \lar \delta^{-\frac{1}{4}}\eps^{\frac{\nu-1}{3}} (T_\infty-t_*)^{\nu - \eta} \lar \delta^{2(\eta-1)-\frac{1}{4}},
\]
...
&= \dfrac{(\delta \beta)^{1/2} + \eps^{1/3}} {\delta^{\frac{-1}{4}}(\delta \beta)^{3/2} + \eps^{2/3}} \dfrac{\eps^\eta}{(\delta \beta + \eps^{2/3} \Omega_0 )^{\eta} }\\
& \lar \dfrac{1} {\delta^{\frac{-1}{4}}(\delta \beta)^{3/2} + \eps^{2/3}} \dfrac{\eps^\eta}{( (\delta \beta)^{1/2} + \eps^{1/3} )^{2\eta -1} } \\
...
\begin{theorem}\label{R-mix}
\[
\|\mathcal{R}_{-,m} \|_{\mathcal{C}_V} \lar \max\{\eps^{\frac{1-\nu}{6}}, \eps^{\frac{\nu}{6}}, \delta^{\frac{1}{4}} \}.
\]
\end{theorem}
\begin{proof}
...
For $0\le t \le T,$ since $|u_+(t)| \lar (T_\infty-t)^{-1} \le (T_\infty-T)^{-1} \lar \delta $, this implies
\[
\|\chi_+u_+W_-^2\| \lar \delta \|W_-^2\| \lar \delta^{\frac{5}{4}},
\]
...
The equation for $W_+$ is
\begin{align*}
W_+'  -2u_+W_+ &=  \chi_+\mu+ 2\chi_+(u_+ -u_-)W_-+ (\chi_-\chi_+) u_+(u_- -u_+)+\frac{1}{2}\chi_+'(u_- -u_+) \\
&+W_+^2+ (\chi_+u_+ + W_+)^3 \\
&+ 3\chi_+^2\chi_-u_+^2u_- + 6\chi_+\chi_- u_+u_- W_+ \\
&+2\chi_+W_-W_++6 \chi_+u_+W_+W_- +3\chi_+(W_-+W_+)W_-W_+\\
&+3(\chi_-u_-)W_+^2 + 3(\chi_+u_+)^2W_-
\end{align*}
...
\[
\|\mathcal{R}_{+,d} \|_{\eta+1}  \lar \eps^{\frac{\eta}{6}}
\]
\end{theorem}

\begin{lemma} \label{DifrEsti2}
...
For $W_-(t)$ with $t>t_*-1$, the estimate
\[ 
|W_-(t)| \lar \delta^{\frac{1}{4}}\eps^{\frac{1-\nu}{3}}(T_\infty-t)^{-\nu}
\]
holds.

Then, use lemma \ref{DifrEsti2}
\begin{align*}
\|2\chi_+(u_+ - u_-)W_- \| &\lar  |(T_\infty-t)^{1+\eta}\chi_+(u_- - u_+)W_-| \lar \\
& \lar \eps^{\frac{1}{2}}(T_\infty-t)^{1+\eta}\delta^{\frac{1}{4}}\eps^{\frac{1-\nu}{3}}(T_\infty-t)^{-\nu} \\
&\lar \eps^{\frac{1}{2}+\frac{1-\nu}{3}}(T_\infty-t_*)^{1+\eta}\delta^{\frac{1}{4}}(T_\infty-T)^{-\nu}\\
&\lar \eps^{\frac{1}{2}+\frac{1-\nu}{3}-\frac{1+\eta}{6}} \delta^{\nu+\frac{1}{4}}\\
&\lar \eps^{\frac{2-\nu}{6}} = \eps^{\frac{\eta}{6}},
\end{align*}
and for the other two terms, we actually have $|u_- - u_+|\lar \eps^{\frac{2}{3}}$, thanks to the term $\chi_-\chi_+$ and $\chi_+'$.
\begin{align*}
\|(\chi_-\chi_+)u_+(u_- -u_+)\|  &\lar (T_\infty-t_*)^\eta\eps^{\frac{2}{3}} \lar \eps^{\frac{4-\eta}{6}} \\
\|(\chi_+')(u_- -u_+)\|  &\lar (T_\infty-t_*)^{1+\eta}\eps^{\frac{2}{3}} \lar \eps^{\frac{3-\eta}{6}}
\end{align*}
\end{proof}
...
\begin{theorem}
\[
\| \mathcal{R}_{+,c} \|_{\eta+1} \lar \max\{\delta^{\eta-1},\delta^{2-\eta}\} 
\]
\end{theorem}

\begin{proof}
The definition of the function space gives
\[
|W_+(t)| \lar (T_\infty - t)^{-\eta}
\]
\begin{align*}
\|W_+^2 \| &\lar |(T_\infty - t)^{1+\eta}W_+(t)^2| \lar (T_\infty- t)^{1+\eta -2\eta} \\
& \lar (T_\infty - T)^{1-\eta} \lar \delta^{\eta-1}
\end{align*}

Similarly for $W_-^3$ term
\begin{align*}
\|W_+^3\| &\lar |(T_\infty-t)^{1+\eta} (T_\infty-t)^{-3\eta}| = (T_\infty-t)^{1-2\eta} \lar (T_\infty-T)^{1-2\eta} \lar \delta^{2\eta-1}\\
\end{align*}
note here $t$ lies in the full range $(0,T)$, since there are no cutoff function multiplied to these terms.

For the term $(\chi_+u_+)^3$, we have
\begin{align*}
\|\chi_+^3u_+^3\| \lar (T_\infty - t)^{1+\eta}u_+^3 \lar (T_\infty - t)^{1+\eta -3} \lar (T_\infty -T)^{\eta-2} \lar \delta^{2-\eta}
\end{align*}
\end{proof}
\item mixed terms
...
\begin{theorem}\label{R+mix}
\[
 \|\mathcal{R}_{+,m}\| \lar \max\{\delta^{2-\eta}, \delta^{\eta-1},\eps^{\frac{1-\nu}{6}} \}
\]
\end{theorem}
 
\begin{proof}
Following the proof of theorem \ref{R-mix}, first
\begin{align*}
\|(\chi_+^2\chi_-)u_+^2u_-\| &\lar (T_\infty-t)^{1+\eta} \left( u_+^3+u_+^2(u_--u_+)\right)\\
&\lar \|u_+^3\|+\eps^{\frac{2}{3}}\|u_+^2\| \lar \delta^{2-\eta} + \eps^{\frac{2}{3}-\frac{\eta-1}{6}}
\end{align*}
by lemma \ref{DifrEsti}.

Second, 
\begin{align*}
\|(\chi_+\chi_-)u_+u_-W_+\| &\lar (T_\infty-t)^{1+\eta}\left(u_+^2+u_+(u_+-u_-)\right)W_+\\
&\lar (T_\infty - t)\left( u_+^2+u_+(u_+-u_-)\right) \\
& \lar (T_\infty-t)^{-1} + \eps^{\frac{2}{3}} \lar (T_\infty-t_*)^{-1} + \eps^{\frac{2}{3}}\\
&\lar  \eps^{\frac{1}{6}} .
\end{align*}

Next are the $W_-W_+$ terms.
\begin{align*}
\|\chi_+W_+W_-\| &\lar (T_\infty - t)^{1+\eta}(T_\infty - t)^{-\eta} \delta^{\frac{1}{4}} \eps^{\frac{1-\nu}{3}} (T_\infty -t)^{-\nu} \\
& \lar (T_\infty - t)^{1-\nu} \eps^{\frac{1-\nu}{3}} \delta^{\frac{1}{4}} \lar (T_\infty -t_*)^{1-\nu}\eps^{\frac{1-\nu}{3}}\delta^{\frac{1}{4}}\\
& \lar \eps^{\frac{1-\nu}{6}},
\end{align*}

\begin{align*}
\|\chi_+(W_-+W_+)W_-W_+\| &\lar (T_\infty - t)^{1+\eta}(T_\infty-t)^{-\nu} \eps^{\frac{2(1-\nu)}{3}}(T_\infty-t)^{-\nu}(T_\infty-t)^{-\eta}+ \\&+(T_\infty-t)^{1+\eta}(T_\infty-t)^{-\eta} \eps^{\frac{1-\nu}{3}}(T_\infty-t)^{-\nu}(T_\infty - t)^{-\eta} \\
&\lar (T_\infty-t)^{1-\nu}(T_\infty-t)^{-\nu}\eps^{\frac{2(1-\nu)}{3}} + (T_\infty-t)^{1-\eta}(T_\infty-t)^{-\nu}\eps^{\frac{1-\nu}{3}}\\
&\lar \eps^{\frac{v-1}{6}}\delta^{\nu}\eps^{\frac{2(1-\nu)}{3}} + \delta\eps^{\frac{1-\nu}{3}} \lar \eps^{\frac{1-\nu}{2}}+\eps^{\frac{1-\nu}{3}},
\end{align*}

\begin{align*}
\|u_+W_-W_+\| &\lar (T_\infty - t)^{1+\eta} (T_\infty-t)^{-1}\delta\eps^{\frac{1-\nu}{3}}(T_\infty-t)^{-\nu}(T_\infty-t)^{-\eta} \\
&\lar \delta\eps^{-\frac{1-\nu}{3}}(T_\infty-t)^{-\nu} \lar \delta^{\nu+\frac{1}{4}}\eps^{\frac{1-\nu}{3}} .
\end{align*}
Finally, for the term $(\chi_-u_-)W_+^2$, because of the cut off $\chi_-$, this term is nonzero only for $t \in (0,t_*-1)$, using lemma \ref{u_-est}, we conclude that 
\[
\| \chi_-u_-W_+^2 \| \lar \|W_+\|^2 \lar \delta^{\eta-1}
\]

The last term $(\chi_+u_+)^2W_-$ satisfies
\begin{align*}
\|(\chi_+u_+)^2W_-\| &\lar (T_\infty-t)^{1+\eta}(T_\infty-t)^{-2} \delta^{\frac{1}{4}}\eps^{\frac{1-\nu}{3}} (T_\infty -t )^{-\nu } \\
& \lar (T_\infty-t)^{\eta-1} \delta^{\frac{1}{4}}\eps^{\frac{1-\nu}{3}}(T_\infty-t)^{-\nu} \\
& \lar (T_\infty-t_*)^{\eta-1} \delta^{\frac{1}{4}}\eps^{\frac{1-\nu}{3}}(T_\infty - T)^{-\nu}\\
& \lar \eps^{\frac{1-\eta}{6}}\eps^{\frac{1-\nu}{3}} \delta^{\frac{1}{4}}\delta^{\nu} \lar \eps^{\frac{\eta-1}{6}},
\end{align*}
